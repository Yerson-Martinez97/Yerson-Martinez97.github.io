document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("json/RestaurantesInfo.json")
    ...
    .then((data) => {
      document
        .querySelectorAll("#linkFerias .card[data-idFeria]")
        .forEach((card) => {
          card.addEventListener("click", function () {
            ...
            if (feria) {
              openModalFeria(
                ...
                feria.businesshours || "",
                feria.reservations || "",
                feria.socialnetworks || "",
                feria.recommendations || "",
                feria.location || "",
                feria.images || []
              );
            } else {
...
function openModalFeria(
  ...
  // =============================================================================================
  // STARS
  const modalFeriastars = document.getElementById("modalFeria-stars");
  ...
  // =============================================================================================
  // OPINIONS
  const modalFeriaOpinions = document.getElementById("modalFeria-opinions");
  modalFeriaOpinions.innerHTML = opinions;
  // =============================================================================================
  // BUSINESS HOURS
  const modalFeriaBusinessHours = document.getElementById(
    "modalFeria-businesshours"
  );
  modalFeriaBusinessHours.innerHTML = ""; // Limpiar horario anterior

  if (businesshours && Array.isArray(businesshours)) {
    const businessHoursTitle = document.createElement("h3");
    businessHoursTitle.innerText = "Horario de Atención:";
    modalFeriaBusinessHours.appendChild(businessHoursTitle);
    ...
    if (Array.isArray(businesshours[1])) {
      ...
      modalFeriaBusinessHours.style.display = "none";
    }

    modalFeriaBusinessHours.appendChild(list);
  } else {
    modalFeriaBusinessHours.style.display = "none";
  }
  // =============================================================================================
  // IMAGES
  const imagesContainer = document.getElementById("modalFeria-images");
  ...
  images.forEach((src, index) => {
    ...
    img.src = src;
    img.alt = title;
    img.classList.add("modalFeria-image");
  ...
  // =============================================================================================
  // RESERVATIONS
  const modalFeriaReservations = document.getElementById(
    "modalFeria-reservations"
  );
  modalFeriaReservations.innerHTML = "";
  ...
  if (Array.isArray(reservations) && reservations.length > 0) {
    reservations.forEach((reservation) => {
      ...
      modalFeriaReservations.appendChild(reservationContainer);
    });
  } else {
    // Si no existen reservas, mostrar un mensaje
    // modalFeriaReservations.innerHTML = "<p>No hay números disponibles.</p>";
  }
  // =============================================================================================
  // LOCATION
  ...
  // =============================================================================================
  // RECOMENDATIONS
  const modalFeriaRecommendations = document.getElementById(
    "modalFeria-recommendations"
  );
  if (Array.isArray(recommendations) && recommendations.length > 0) {
    ...
    modalFeriaRecommendations.innerHTML =
      "<div class='title'>Recomendaciones</div>";
    modalFeriaRecommendations.appendChild(ul);
  } else {
    modalFeriaRecommendations.innerHTML = "Información no disponible.";
  }
}
